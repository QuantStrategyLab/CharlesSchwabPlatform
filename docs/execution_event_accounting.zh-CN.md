...
订单累计成交数量只用于和已记录的增量成交合计核对，绝不被转换成成交；乱序订单更新只保留累计数量高水位，不接受下降。`PENDING_CANCEL` 仍是非终态。终态累计数大于（或不等于）已记录的增量成交时，状态为 `incomplete`，预留保持 `pending`；补齐成交流后会重新核对。同 ID 冲突会隔离该事件及其订单，使订单保持 `incomplete` 并保留预留，但不改写已入账成交。确认撤单不会冲掉之前已证明的成交。成交、费用、owner 份额和待结算条目按一个事件各记一次；待结算条目只表示尚待结算，不代表可用现金或已结算余额。

账本以确定性 JSON 快照保存意图、订单状态、事件、隔离事件、owner 经济账和账户合计。订单身份未知的 intent 预留仍归属原 owner 并计入账户预留；绑定精确订单 ID 后从未绑定项移入订单项，不会重复累计。每个 owner 的预留金额独立累计，账户总预留是 owner 预留之和；一个 owner 的订单不会引用另一个 owner 的预留。临时文件写入并 `fsync` 后通过 `os.replace` 原子替换，再同步目录。重启从快照重建状态，同一事件重放保持不变。这只证明本地快照和账本层的幂等；不证明网络 exactly-once、跨主机 fencing 或券商业务恢复。
