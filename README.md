...
The `tqqq_growth_income` strategy implementation is sourced from `UsEquityStrategies` (legacy alias: `hybrid_growth_income`).
...
`tqqq_growth_income` 策略实现来自 `UsEquityStrategies`（兼容别名：`hybrid_growth_income`）。
