...
| `CRISIS_ALERT_GOOGLE_VOICE_GMAIL_APP_PASSWORD` | Gmail App Password for the sender account. For Cloud Run, prefer `CRISIS_ALERT_GOOGLE_VOICE_GMAIL_APP_PASSWORD_SECRET_NAME` in env sync. |
...
| `CRISIS_ALERT_GOOGLE_VOICE_GMAIL_USER` | 可选的 Google Voice 通知 Gmail 地址。 |
| `CRISIS_ALERT_GOOGLE_VOICE_GMAIL_APP_PASSWORD` | 发送方 Gmail 账号的 App Password。Cloud Run env sync 建议配置 `CRISIS_ALERT_GOOGLE_VOICE_GMAIL_APP_PASSWORD_SECRET_NAME`。 |
