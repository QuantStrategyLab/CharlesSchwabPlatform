...
class RebalanceServiceTests(unittest.TestCase):
    ...
    def test_strategy_risk_rejection_keeps_reason_with_zero_submissions(self):
        ...
        self.assertEqual(result.execution["no_op_reason"], "rejected:too_many_positions")
    ...
    def test_dry_run_bypass_execution_marker_continues_rebalance(self):
        ...
        from application.rebalance_service import DRY_RUN_BYPASS_EXECUTION_MARKER_ENV

        os.environ[DRY_RUN_BYPASS_EXECUTION_MARKER_ENV] = "true"
        self.addCleanup(lambda: os.environ.pop(DRY_RUN_BYPASS_EXECUTION_MARKER_ENV, None))
        ...
        class FakeStore:
            ...
            def claim_marker(self, marker_key, *, metadata=None):
                raise AssertionError("bypass must not claim markers")

            def read_marker(self, marker_key):
                return None

            def record_marker(self, *_args, **_kwargs):
                raise AssertionError("bypass must not record markers")

            def has_prior_execution_report(self, **_kwargs):
                raise AssertionError("bypass must not read prior reports")
