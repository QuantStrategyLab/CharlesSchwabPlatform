...
grep -Fq 'python3 scripts/reconcile_cloud_runtime.py build-monitor-targets' "$workflow_file"
grep -Fq 'direct_monitor_description="Managed by CharlesSchwabPlatform direct-monitor-v1"' "$workflow_file"
grep -Fq -- '--description="${direct_monitor_description}"' "$workflow_file"
...
deploy_block="$(sed -n '/gcloud run deploy "${CLOUD_RUN_SERVICE}"/,/--quiet/p' "$workflow_file")"
grep -Fq -- '--ingress=internal' <<<"$deploy_block"
grep -Fq -- '--no-allow-unauthenticated' <<<"$deploy_block"
grep -Fq '"--workers", "1", "--threads", "1"' "$dockerfile"
grep -Fxq 'threads = 1' "$gunicorn_config"
...
main_scheduler_update_block="$(sed -n '/gcloud scheduler jobs update http "${job_name}"/,/--quiet/p' "$workflow_file")"
grep -Fq -- '--http-method=POST' <<<"$main_scheduler_update_block"
grep -Fq -- '--oidc-service-account-email="${GCP_SCHEDULER_SERVICE_ACCOUNT}"' <<<"$main_scheduler_update_block"
grep -Fq -- '--oidc-token-audience="${service_url}"' <<<"$main_scheduler_update_block"
main_scheduler_create_block="$(sed -n '/gcloud scheduler jobs create http "${job_name}"/,/--quiet/p' "$workflow_file")"
grep -Fq -- '--http-method=POST' <<<"$main_scheduler_create_block"
grep -Fq -- '--oidc-service-account-email="${GCP_SCHEDULER_SERVICE_ACCOUNT}"' <<<"$main_scheduler_create_block"
grep -Fq -- '--oidc-token-audience="${service_url}"' <<<"$main_scheduler_create_block"
